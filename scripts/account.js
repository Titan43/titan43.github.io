...
function updateUser(username, data) {
  const cookies = Object.fromEntries(document.cookie.split('; ')
      .map((c) => c.split('=')));
  return fetch(`${USER_LINK}?username=${username}`, {
    method: 'PUT',
    headers: {
...
function deleteAccount(username) {
  const cookies = Object.fromEntries(document.cookie.split('; ')
      .map((c) => c.split('=')));
  fetch(`${USER_LINK}?username=${username}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${cookies.token}`,
...
obtainUser().then((data) => {
  ...
  updateAccButton.onclick = function() {
    if (!document.querySelector('form')) {
      itemsSection.insertBefore(
          updateUserForm(data.username),
          itemsSection.children[1]);
    }
  };

  itemsSection.replaceChild(buildUserBlock(data),
